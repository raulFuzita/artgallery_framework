...
function loadContent(){
    ...
    Controller = "../App/Controller/Art/Favorite/LoadArts.Controller.php";
...
function drawPagination(currentPage){
    // Load the Pagination
    Controller = "../App/Controller/Art/Favorite/LoadPagination.Controller.php?page="+currentPage;
...
function onclickDetected(target){
    ...
    if(targetClass == "page"){
        ...
        // Load all content from a database table
        Controller = "../App/Controller/Art/Favorite/PageArts.Controller.php?page="+number;
        displayTo = "ajax-content";
        ajaxRequest(displayTo, Controller);
...
function searchContent(text){
    ...
    if(text.length == 0){
        ...
        // Load all content from a database table
        Controller = "../App/Controller/Art/Favorite/SearchArts.Controller.php?search="+text;
        displayTo = "ajax-content";
        ajaxRequest(displayTo, Controller);
    }
}
...
function validateId(pageId){

    pageId = parseInt(pageId);
   
    if(!isNaN(pageId)){
        return (pageId <= 1) ? 1 : pageId;
    } else if(pageId == previous || pageId == next){

        let element = document.getElementsByClassName(pagination);
        let parentElement = document.getElementsByClassName('page-item active');
        let index = parentElement.value;
        let size = element.length;

        if(size > 2){
            
            max = (index < (size-1)) ? index++: index;
            min = (index > 0) ? index--: 0;
            return (pageId == previous) ? min : max;
        }
    }
    return 0;
}
